...
WHERE {{
  GRAPH <{TARGET_GRAPH}> {{
    ...
    def delete_concept(concept_identifier):
        ...
        # Split deletion into smaller updates following the actual URI shapes generated by create_uri()
        # and skolem templates, to reduce timeout/disconnect risk.
        delete_steps = [
            (
                "concept-identity",
                f'isIRI(?s) && STR(?s) = "{concept_base}"',
            ),
            (
                "concept-versions",
                f'isIRI(?s) && STRSTARTS(STR(?s), "{concept_base}/version/")',
            ),
            (
                "all-root",
                f'isIRI(?s) && STR(?s) = "{concept_base}/all"',
            ),
            (
                "all-versions",
                f'isIRI(?s) && STRSTARTS(STR(?s), "{concept_base}/all/version/")',
            ),
            (
                "levels",
                f'isIRI(?s) && STRSTARTS(STR(?s), "{concept_base}/level_")',
            ),
            (
                "skolem",
                f'isIRI(?s) && STRSTARTS(STR(?s), "{concept_base}/.well-known/genid/")',
            ),
            (
                "codes-and-code-versions",
                f'isIRI(?s) && STRSTARTS(STR(?s), "{concept_base}/") '
                f'&& !STRSTARTS(STR(?s), "{concept_base}/version/") '
                f'&& !STRSTARTS(STR(?s), "{concept_base}/all") '
                f'&& !STRSTARTS(STR(?s), "{concept_base}/level_") '
                f'&& !STRSTARTS(STR(?s), "{concept_base}/.well-known/")',
            ),
        ]
...
"""
        ...
        for step_name, step_filter in delete_steps:
            print(f"DEBUG: delete_concept step={step_name} concept={concept_identifier}")
            LindasAPIHelper.graphdb_update(build_delete_query(step_filter))
