...
class GraphManager:
    def __init__(self, base_uri, output_file=OUTPUT_FILE_NAME, enable_skolem=True):
        ...
        self.graph = StreamingTurtleWriter(output_file, enable_skolem=enable_skolem)
...
class CatalogManager:
    ...
    def _add_dataset_description(self, uri, titles, descriptions, publication_date, example_resource, modified_date=None):
        ...
        sparql_example = BNode()
        self.vm.graph.add((uri, SDO.workExample, sparql_example))
        self.vm.graph.add((sparql_example, RDF.type, SDO.CreativeWork))
        self.vm.graph.add((sparql_example, SDO.encodingFormat, Literal("text/html")))
        self.vm.graph.add((sparql_example, SDO.url, self.SPARQL_EXAMPLES_URL))
        for language, name in {
            ...
            self.vm.graph.add((sparql_example, SDO.name, Literal(name, lang=language)))
